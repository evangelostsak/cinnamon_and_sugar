...
const ANALYTICS = {
  ...
  cloudflare: '  <!-- Cloudflare injects its analytics beacon at the edge. -->'
};
...
function buildPage(page, base, dict) {
  ...
  caller = caller.replace(/<!-- build:analytics -->[\s\S]*?<!-- \/build:analytics -->/,
    "<!-- build:analytics -->\n" + ANALYTICS[HOST] + "\n  <!-- /build:analytics -->");
...
/* Cloudflare Pages reads _headers and _redirects instead of vercel.json. Generating
   them keeps one source of truth, so the two platforms cannot drift apart.
   Cloudflare applies every matching rule in order, later ones winning, so the
   catch-all goes first and the asset overrides after it. */
const CF_ANALYTICS = "https://static.cloudflareinsights.com";

function cfPattern(source) {
  // "/(.*)" -> "/*",  "/source/images/(.*)" -> "/source/images/*"
  const alt = source.match(/^\/(?:source\/)?\(([a-z0-9|/]+)\)\/\(\.\*\)$/);
  if (alt) {
    const prefix = source.startsWith("/source/") ? "/source/" : "/";
    return alt[1].split("|").map(part => prefix + part + "/*");
  }
  if (source === "/(.*)") return ["/*"];
  if (source.endsWith("/(.*)")) return [source.slice(0, -5) + "/*"];
  return null;                       // e.g. "/(.*)\.html" — no Cloudflare equivalent
}

function writeCloudflare(cfg) {
  const lines = [];
  for (const rule of cfg.headers || []) {
    for (const pattern of cfgPatterns(rule.source)) {
      lines.push(pattern);
      for (const { key, value } of rule.headers) {
        let v = value;
        // the analytics beacon is a different origin on Cloudflare than on Vercel
        if (key === "Content-Security-Policy")
          v = v.replace("script-src 'self'", "script-src 'self' " + CF_ANALYTICS)
  ...
  write(path.join(ROOT, "_headers"),
    "# Generated by tools/build.js from vercel.json — do not edit by hand.\n\n" + lines.join("\n"));

  const red = (cfg.redirects || []).map(r =>
    `${r.source} ${r.destination} ${r.permanent ? 301 : 302}`).join("\n");
  write(path.join(ROOT, "_redirects"),
    "# Generated by tools/build.js from vercel.json — do not edit by hand.\n" + red + "\n");
}

function cfgPatterns(source) {
  const p = cfPattern(source);
  if (p) return p;
  return [];                          // silently skipped: the catch-all already covers it
}
...
function main() {
  ...
  const cspOk = syncCsp(built);
  writeCloudflare(JSON.parse(read(ROOT, "vercel.json")));
  if (!cspOk && !check) process.exit(1);
}
