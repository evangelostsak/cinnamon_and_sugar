...
/* Every inline <script> in the built pages, hashed for the CSP. */
...
/* Inline scripts are allowed by hash. Cloudflare reads _headers when the
   deployment is created, so a hash fixed here lands on the next deploy — hence
   the non-zero exit, which fails the build rather than shipping a page whose
...
function syncCsp(pages) {
  ...
  /* Anchored to the header line, so a stray mention elsewhere in the file
     cannot be rewritten instead. */
  const after = before.replace(
    /(^ +Content-Security-Policy:[^\n]*?script-src )([^;]*)/m,
    (whole, head) => head + "'self' https://static.cloudflareinsights.com" +
                     (wanted ? " " + wanted : ""));

  if (after === before) return true;
  fs.writeFileSync(file, after);
  console.error("\n  _headers: CSP script hashes were stale — rewritten.");
  console.error("  Commit _headers and redeploy; this build serves the old policy.");
  return false;
}
